#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ptfx_build_catalog.py — katalogdaki bir efekt ailesini uretir ve DENETLER.
...
Kullanim:
  python ptfx_build_catalog.py --grup virus_ambiyans
  python ptfx_build_catalog.py --grup virus_ambiyans --denetle-sadece
"""
...
def kur(aile, donor, sprite, renk, ovr, neden, hareket, dizin):
    ...
    if sprite in BE:
        ...
        rb = ps(sys.executable, os.path.join(BETIK, "ptfx_blender_energy.py"),
    ...
    if uretici not in ptfx_sheet.AILE:
        return "sprite uretici yok: %s" % uretici
    png = os.path.join(dizin, ad + ".png")
    dds = os.path.join(dizin, ad + ".dds")
    try:
        tekil = uretici in getattr(ptfx_sheet, 'TEKIL', set())
        ptfx_sheet.yaz(
            ptfx_sheet.tek_kare(uretici, COZ, t, tohum, notr, tekil), png)
...
def _kur_devam(aile, donor, renk, ovr, neden, hareket, dizin, ad):
    ...
    s, zn = zarf_onar(s)
    if hareket:
        s, hn = ptfx_motion.uygula_belge(s, hareket)
        zn += hn
