...
- **`specialAttribute` kapı tablosu:** 7 menteşeli · 8 sürgülü · 5 garaj · 10 kepenk · 12 bariyer → kapı sistemi
  çalışır; **0 = kapı DEĞİL**, kayıt olsa bile kımıldamaz. Tam 21 değer `trunk/flags.md`.
...
- Sollumz: `bpy.data.objects.new` ile obje `sollum_type` taşımaz → dosya hiç yazılmaz; `sz_lods.high.mesh`;
  `holes_fill` UV (0,0) örnekler; ölçek en **büyük** eksene → `trunk/tool-pitfalls.md` §1-2.
...
- **Ele tutturma:** `PH_R_Hand` 28422 · `PH_L_Hand` 60309 · `SKEL_L_Hand` 18905; burger klibi **sol** elle yer.
  Ofset tablosu `rotationOrder`'ıyla taşınır (dpemotes 1 · ox_lib/qb 0) → `hand-attach.md`.
...
- **Script'le spawn edilen obje harita objesi değildir**; MLO içine ymap ile prop konmaz → `branches/map/_branch.md`.
...
|---|---|---|
| Kapı açılmıyor / obje kımıldamıyor / kilit / koordinat | doors-and-motion.md | ölçüldü — eski SKILL kapı/hareket/override/koordinat |
| Kırılabilir / kemikli prop (fragment) | fragment.md | ölçüldü — eski SKILL fragment üretimi |
| Objenin üstünde canlı ekran (DUI) | dui-screens.md | ölçüldü — 3dnui-dui-panel |
| Prop'u kemiğe tutturma — ofset tablosu taşıma, rotationOrder, canlı ofset editörü | hand-attach.md | okundu + sayısal — pg_attachproptoplayereditor, dpemotes, ox_lib, qb progressbar |

## Gövdeye bakılacaklar
- `trunk/flags.md` — `specialAttribute` 21 değer, archetype bayrakları, extension tipleri (partikül, ladder, buoyancy)
- `trunk/verification-ladder.md` — prop'u oyuna sokmadan: export boyutu, bound, ytyp geri okuma
- `trunk/tool-pitfalls.md` §1 Sollumz, §2 Blender (transform pişir), §6 FiveM (`CreateModelHide`, handle önbelleği)
- Prop'un ışığı → `branches/look/lights.md`
- Hazır örnek / araç arıyorsan (3 parçaya ayrılan `yft.blend`, `blender_rayfirev`, drawable import temizliği, ⛔ boş `.col` = anında crash) → `sources/community-resources.md` §5, §8 — **kaynak notu, kural değil.**
