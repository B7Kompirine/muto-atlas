...
| Işık — oku/düzenle/geri yaz, TimeFlags, Flashiness, gobo | lights.md | ölçüldü — isik-mat · decal-isik §4,9 · HATALAR |
| Işık matematiği ve Blender önizleme | light-math.md | ölçüldü — isik-mat §1-8 |
...
- `trunk/tool-pitfalls.md` §1 (gömülü doku DDS, `use_custom_settings`, koni radyan, ışık export), §2 (`.color_srgb`, bisect, `bound_box`),
- `trunk/verification-ladder.md` — ışık/doku/decal'i oyuna sokmadan: `.ydr` ışık geri okuma, `.ytd` boyut/format
  §3 (`.ytd` şeması, `GetXMLFormat`, RPF anahtar)
- `trunk/flags.md` (Time arketipi, Cast Shadows, Dont Render In Reflections) ·
  iç mekân bütçesi `branches/map/vanilla-interiors.md`
- Hazır şablon / araç arıyorsan (Substance GTA baker, timecycle birleşimi, lightshaft `flags 99`, vertex color master, R\* iç mekân renk şeması) → `sources/community-resources.md` §1-3, §9 — **kaynak notu, kural değil.**
