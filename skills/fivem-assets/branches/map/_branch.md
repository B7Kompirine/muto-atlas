...
- ymap yazmanın iki yolu var ve hangisinin kayıpsız olduğu dosyaya göre değişir; `CalcFlags()`
  çağırma, `CalcExtents()` kullanma, `CEntityDefs`'i doğrudan yazma (`trunk/tool-pitfalls.md` §3).
...
|---|---|---|
| Vanilla parçayı kendi modelimle değiştir (yol, prop, yapı) | vanilla-part-replacement.md | ölçüldü — vanilla-parca-degistirme |
| Yıkım / çökme / koreografili sahne — tek yaprak | destruction.md | ölçüldü — rayfire-des-uretim |
| MLO objesini değiştir (entity swap) | mlo-object-swap.md | ölçüldü — mlo-obje-degistirme · eski SKILL |
| MLO'ya kendi prop'unu / drawable'ını koy | mlo-props.md | ölçüldü — mlo-prop-uretim-hatti · mlo-drawable-export |
| LOD zinciri — uzakta titriyor / kayboluyor | lod.md | ölçüldü+video — ytyp-ymap §9 · notlar/08 §8 |
| Vanilla iç mekân ölçüsü | vanilla-interiors.md | ölçüldü — vanilla-ic-mekan-olcumu |
| Çim / prosedürel zemin / arazi | grass-procedural.md | video+veri — notlar/08 §1-7 |

## Gövdeye bakılacaklar
- `trunk/flags.md` — entity/archetype bayrakları, `specialAttribute`, extension'lar
- `trunk/tool-pitfalls.md` §3 CodeWalker (ymap yazma, `MetaHash`, `BoxMin`), §4 PowerShell (`-Flatten`, `[script]`)
- `trunk/verification-ladder.md` — oyuna girmeden ymap/ytyp denetimi
...
- Işık/timecycle/parallax: `branches/look/_branch.md`
- Hazır şablon / araç arıyorsan (LOD şablonları + `nametables.rpf`, Arbolito, VichoTools, `str_enableFlush`, iç mekân referans kütüphaneleri, `Procedural_IDs.txt`) → `sources/community-resources.md` §6, §10, §12 — **kaynak notu, kural değil.**
