...
**Önce:** `branches/particle/_branch.md` · gövde › `trunk/tool-pitfalls.md` §3 CodeWalker (`FxcFileHash`, `VFT`, `ResourcePointerArray64`) · betikler `ptfx_build_catalog.py`, `ptfx_quality_gate.py`, `ptfx_find_donor.py`, `ptfx_merge.py`
...
```
python ptfx_build_catalog.py    --grup <grup>   # üretir
python ptfx_quality_gate.py  --grup <grup>   # vanilla'ya karşı denetler
```
...
popluyor — toplam **%14,7**. Üretici (`ptfx_build_catalog.py: zarf_onar`)
...
göre yapılır, ad benzerliğine göre değil: `ptfx_find_donor.py --ara <terim>`
...
Araç: `ptfx_merge.py --ad my_efektler --klasor <dizin>`
...
`ptfx_find_donor.py` artık bunları eliyor.
