...
**Kaynak:** `ptfx-flipbook-uretimi.md` §1-6, §8, §13 · `trunk/flags.md` §7c-7d (2026-08/09) · **Ölçüm:** `core.ypt` 45,5 MB açıldı; 107 gömülü doku DXT; 15 aile oyunda; sayfa bölünmesi görsel kanıtla
**Önce:** `branches/particle/_branch.md` · gövde › `trunk/tool-pitfalls.md` §3 CodeWalker (`FxcFileHash`, `VFT`, `ResourcePointerArray64`) · katalog/aile üretimi `catalog.md` · katmanlı efekt `layered-effects.md`
...
```powershell
scripts\ptfx_make.ps1 -Ad my_spor -Renk 0.2,0.9,0.35 -Hedef <stream klasoru>
scripts\ptfx_make.ps1 -Ad my_x -Gorsel kendi.png -Boyut 0.4 -Yukselme 0.8 -Hedef <k>
```
...
| Betik | İş |
|---|---|
| `ptfx_make.ps1` | **giriş noktası** — hattın tamamı |
...
| `ptfx_preview.py` | derlenmiş `.ypt`'nin verisinden animasyonlu önizleme (motor çıktısı DEĞİL) |
...
Araç: `ptfx_motion.py` (`uygula_belge`). Attractor **silinmez, yarıçapı
...
Kare sayısı, `C4`, ızgara, yoğunluk, transplant ve donör seçimi → `sprite-sheets.md`.
