...
Sollumz .ycd export (XML cikar)
  -> fix_ycd_xml.py          <Hash> yazilmamis olur
...
`yama_ycd.py`'nin ölçtüğü beş alan (5 vanilla sözlük / 27 klip, **istisna
yok**): `anim.Hash` = klip adı · `clip.Name` = `pack:/<ad>.clip` ·
`clip.Unknown30` = 1 · `anim.Unknown10` = 1 · `clip.Properties` = tek öge.
⛔ Bu betik hattın parçasıdır; "elle hizalarım" deyip atlanınca
...
`119` = `RotX|RotY|RotZ|TransX|TransY|TransZ` (1+2+4+16+32+64).
`4215` = `119 | Unk0(4096)` — **kök kemiğe bu yazılır**, diğerlerine `119`.
...
Denetim + otomatik düzeltme: `scripts/ydr_anim_denetle.ps1 -Ydr x.ydr [-Duzelt]`.
...
Betikler (ikisi de depoda, sırayla): `scripts/yama_ycd.py` klip alanlarını
vanilla sırasıyla yeniden kurar · `scripts/ycd_track2_ekle.py` Track 2
bloklarını `SequenceData`'ya ekler · `scripts/ycd_kok_kemik_ekle.py` kök
kemiğin (tag 0) üç kanalını her track grubunun başına yazar.
