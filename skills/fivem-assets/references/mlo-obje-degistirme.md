# MLO İÇİNDEKİ BİR OBJEYİ KENDİ MODELİMİZLE DEĞİŞTİRME
...
- **`-Command` kullan, `-File` değil.** `-File` ile virgüllü liste TEK
  STRING olarak gider ve sessizce uydurma bir ada dönüşür
  (`muto_teldr,v_ilev_gb_vauldr=muto_vauldr` gibi).
...
> `skills/fivem-assets/references/mlo-obje-degistirme.md` reçetesini uygula.
