...
Komut: `/vehicle` · Klasör: `dallar/vehicle/`
...
- **Araç kemiklerinde AD sabittir, %100 tag-kararlı** (193 ad: kapı/cam/kaput/tekerlek/ışık/motor/egzoz/koltuk/mod/extra/siren) —
  adı birebir kopyala, `GetEntityBoneIndexByName` ile bul. Silah ve ped'de tersi olabilir → `govde/kemik-tag.md` §1 karar kuralı.
- **Künye sorgulanır, tahmin edilmez:** `assetdb.py vehicle <ad>` → `handlingId`, modkit, extra, sınıf, koltuk (921 araç).
- Dış araç bir sayı/tablo getirdiyse (Five Toolkit debadger, tint) **iddiadır** → `kaynaklar/dis-arac.md` etiket sistemi.
- Araç dokusu/shader'ı (`vehicle_paint*`, livery) için doku kuralları `dallar/look/_dal.md`; araç collision bayrakları `govde/bayraklar.md` §8.

## Yapraklar

| istenen | dosya | durum — kaynak |
|---|---|---|
| Kemik / mod / extra / handling sorgusu | kemik-mod.md | ölçüldü — govde/kemik-tag §2 · assetdb vehicle |
| Dış araç — debadger, ayıklanan araç videoları | dis-arac.md | dış kaynak — govde/dis-arac §1k · sollumz-discord (ayıklandı) |

## Gövdeye bakılacaklar
- `govde/kemik-tag.md` §2 (tam tablo), §6 (kararsız adlar) · `govde/bayraklar.md` §8 collision · `kaynaklar/dis-arac.md` §1k
- `govde/dogrulama-merdiveni.md` — iskelet/künye geri okuma
