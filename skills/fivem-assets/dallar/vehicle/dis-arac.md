# Dış araç — Vehicle Debadger, Sollumz araç videoları (ayıklandı)

**Ne zaman okunur:** araç modeline/logosuna dış araçla dokunacaksın; topluluk araç videolarını arıyorsun.
**When to read:** touching a vehicle model or logo with a third-party tool; looking for community vehicle tutorials.
**Kaynak:** `kaynaklar/dis-arac.md` §1k · Sollumz Discord çıkarımı (araç videoları **istek üzerine ayıklandı**, atlas'a alınmadı) · **Ölçüm:** yok
**Önce:** `dallar/vehicle/_dal.md`

---

## Vehicle Debadger (Five Toolkit)

Ölçülmüş gözlem ve etiketleri → `kaynaklar/dis-arac.md` §1k.


## Ayıklanan araç videoları (Sollumz Discord)

Bu içerik atlas'a **bilerek alınmadı**; ihtiyaç olursa yerel çıkarımdan okunur (`Desktop\FiveM\sollumz-tutorials\`): `R5T7stYU3A0` vehicle tint · `w1JUh66MP8I` window tint · `4fHDu6fUw74` siren · `_zH0QaFmFz0` bulletproof glass · `j61xy_BoDNA` shattermap fix · `FZfkuJj5yx0` + `nYeVr126qIk` polis lambaları · `JtAWRNCIT-k` Lola araç yapımı · tuning parça offset düzeltmeleri. `notlar/04` §1 araç kurulumu.
