...
**Kaynak notu — kural değil.** Dış araçların (Five Toolkit vb.) ölçülebilen kısmı ve [doğrulandı]/[çürütüldü] etiket sistemi. Taşındı: `kaynaklar/dis-arac.md` (2026-09-05). Ham kopyalar `_dis-kaynak/` yanında.
...
| çim dört sistem, arazi anatomisi, `@ma`, grass batch, LOD iki mekanizma, Sketchfab animasyonlu drawable | `08-cim-prosedurel-lod-zinciri.md` (→ `dallar/map/cim-prosedurel.md`, `lod.md`) |
| araç eklenti envanteri | `09-arac-eklenti-envanteri.md` (→ `dallar/vehicle/`) |
...
> Shader = program + render kovası ölçümü → `dallar/look/shader.md`.
