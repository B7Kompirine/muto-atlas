...
araç davranışı ayrı: `SKILL.md` ve `govde/arac-tuzaklari.md`.
...
Ayrıntı `govde/arac-tuzaklari.md` Ayrıntı B.
...
- `.ycd` klibi **`<Hash>` ile bulunur**, adla değil: alan boşsa klibin adı yok
  ...
  hangi aracın bu alanı doldurup doldurmadığı → `govde/arac-tuzaklari.md` §1.
...
`dds_to_ytd.ps1` (klasör → `.ytd`, **geri okur**) · `ytd_index.ps1` / `ytd_ara.ps1`
...
**Girmez:** tek bir aracın davranışı (`arac-tuzaklari.md`), bizim çalışma
disiplinimiz (`SKILL.md`), bir kategoriye özgü kural (`dallar/<dal>/_dal.md`),
